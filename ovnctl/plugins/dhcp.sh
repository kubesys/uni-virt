...
kubectl ko nbctl dhcp-options-set-options $uuid lease_time=3600 router=$ip server_id=$ip server_mac=c0:ff:ee:00:00:01 ref=ovn-default

#iface=$(ovs-vsctl get interface $name external_ids:iface-id | sed s/\"//g)
#kubectl ko nbctl lsp-set-dhcpv4-options $iface $uuid