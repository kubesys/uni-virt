...
func NewRevertInternalSnapshotCommand() *cli.Command {
	return &cli.Command{
		...
		Action:    backrevertInternalSnapshot,
		Flags: []cli.Flag{
			&cli.StringFlag{
				Name:  "type",
...
func backrevertInternalSnapshot(ctx *cli.Context) error {
	err := revertInternalSnapshot(ctx)
...
func revertInternalSnapshot(ctx *cli.Context) error {
	...
	if err := virsh.RevertInternalSnapshot(domainName, ctx.String("snapshot")); err != nil {
		logger.Errorf("RevertInternalSnapshot err:%+v", err)
		return err
	}
